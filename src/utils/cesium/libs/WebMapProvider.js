...
function WebMapTileServiceImageryProvider(options) {
	...
	if (!Cesium.defined(options.layer)) {
		throw new Cesium.DeveloperError("options.layer is required.");
	}
	...
	if (!Cesium.defined(bracketMatch) || (bracketMatch.length === 1 && /{s}/.test(url))) {
	...
	this._format = Cesium.defaultValue(options.format, "image/jpeg");
	...
	const swTile = this._tilingScheme.positionToTileXY(Cesium.Rectangle.southwest(this._rectangle), this._minimumLevel);
	const neTile = this._tilingScheme.positionToTileXY(Cesium.Rectangle.northeast(this._rectangle), this._minimumLevel);
	...
	} else if (Cesium.defined(this._subdomains) && this._subdomains.length > 0) {
...
function requestImage(imageryProvider, col, row, level, request, interval) {
	...
	const tileMatrix = Cesium.defined(labels) ? labels[level] : level.toString();
	const subdomains = imageryProvider._subdomains;
	const staticDimensions = imageryProvider._dimensions;
	const dynamicIntervalData = Cesium.defined(interval) ? interval.data : undefined;
	...
	dimensions: {
		...
		set: function (value) {
			if (this._dimensions !== value) {
				...
				if (Cesium.defined(this._reload)) {
					this._reload();
				}
			}
		},
...
WebMapTileServiceImageryProvider.prototype.requestImage = function (x, y, level, request) {
	...
	let currentInterval;
	if (level <= 5) {
		result = requestImage(this, x, y, 5, request);
	}
