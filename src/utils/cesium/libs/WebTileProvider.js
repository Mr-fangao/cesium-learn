 ...
 * @property {Rectangle} [rectangle=Cesium.Rectangle.MAX_VALUE] The rectangle, in radians, covered by the image.
...
function UrlTemplateImageryProvider(options) {
	...
	if (!Cesium.when.isPromise(options) && !Cesium.defined(options.url)) {
		throw new Cesium.DeveloperError("options is required.");
	}
	//>>includeEnd('debug');
...
UrlTemplateImageryProvider.prototype.reinitialize = function (options) {
	const that = this;
	that._readyPromise = Cesium.when(options).then(function (properties) {
		//>>includeStart('debug', pragmas.debug);
...
UrlTemplateImageryProvider.prototype.requestImage = function (x, y, level, request) {
	...
	// tms切片规则转 wmts
	// y = Math.pow(2, level) - y - 1;
	//author:dongnan 这里就是跳过前面5级，根据自己需要设置
	if (level <= 5) {
		return Cesium.ImageryProvider.loadImage(this, buildImageResource(this, x, y, 5, request));
	}
