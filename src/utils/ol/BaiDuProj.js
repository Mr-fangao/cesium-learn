...
bd09.toWGS84 = function (input, opt_output, opt_dimension) {
	let output = forEachPoint(toGCJ02)(input, opt_output, opt_dimension);
...
let baiduMercatorProj = new ol.proj.Projection({
	code: "baidu",
	extent: ol.extent.applyTransform(extent, projzh.ll2bmerc),
	units: "m",
});
export function initBaiduProj() {
	ol.proj.addProjection(baiduMercatorProj);
	ol.proj.addCoordinateTransforms(ol.proj.get("EPSG:4326"), baiduMercatorProj, projzh.ll2bmerc, projzh.bmerc2ll);
	ol.proj.addCoordinateTransforms(ol.proj.get("EPSG:3857"), baiduMercatorProj, projzh.smerc2bmerc, projzh.bmerc2smerc);
}
