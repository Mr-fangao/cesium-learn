...
const fs = `
 ...
 vec4 getPositionEC(){
     return czm_windowToEyeCoordinates(gFragColor);
 }
 vec3 getNormalEC(){
     return vec3(1.);
 }
 ...
 void main(){
     // 釉色 = 结构二维(颜色纹理, 纹理坐标)
     gFragColor = texture(colorTexture, v_textureCoordinates);
     ...
     float dis = length(vcPos.xyz/vcPos.w);
     if(dis > near && dis < helsing_viewDistance){
         ...
         if(visible(posInEye)){//在虚拟相机中该片源也是可见的
             float vis = shadow(viewPos);
             if(vis > 0.3){
                gFragColor = mix(gFragColor,helsing_visibleAreaColor,.5);
             } else{
                gFragColor = mix(gFragColor,helsing_invisibleAreaColor,.5);
             }
         }
...
class Viewshed {
	constructor(viewer, options) {
		this.viewer = viewer;
		this.viewPosition = options && options.viewPosition ? options.viewPosition : viewer.camera.position.clone(new Cesium.Cartesian3());
		this.viewPositionEnd = options && options.viewPositionEnd ? options.viewPositionEnd : null;
		this.viewDistance = this.viewPositionEnd ? Cesium.Cartesian3.distance(this.viewPosition, this.viewPositionEnd) : (options ? (options.viewDistance || 100.0) : 100);
		this.viewHeading = this.viewPositionEnd ? getHeading(this.viewPosition, this.viewPositionEnd) : (options ? (options.viewHeading || 0.0) : 0.0);
		this.viewPitch = this.viewPositionEnd ? getPitch(this.viewPosition, this.viewPositionEnd) : (options ? (options.viewPitch || 0.0) : 0.0);
		this.horizontalViewAngle = options ? (options.horizontalViewAngle || 90.0) : 90.0;
		this.verticalViewAngle = options ? (options.verticalViewAngle || 60.0) : 60.0;
		this.visibleAreaColor = options ? (options.visibleAreaColor || Cesium.Color.GREEN) : Cesium.Color.GREEN;
		this.invisibleAreaColor = options ? (options.invisibleAreaColor || Cesium.Color.RED) : Cesium.Color.RED;
		this.enabled = options ? ((typeof options.enabled === "boolean") ? options.enabled : true) : true;
		this.softShadows = options ? ((typeof options.softShadows === "boolean") ? options.softShadows : true) : true;
		this.size = options ? (options.size || 2048) : 2048;
		this.originshadowmap=new Cesium.ShadowMap({
			context: (this.viewer.scene).context,
			lightCamera: this.viewer.scene.shadowMap._lightCamera,
			enabled: this.viewer.scene.shadowMap.enabled,
			isPointLight: this.viewer.scene.shadowMap.isPointLight,
			pointLightRadius: this.viewer.scene.shadowMap._pointLightRadius,
			cascadesEnabled: this.viewer.scene.shadowMap._cascadesEnabled,
			size: this.viewer.scene.shadowMap.size,
			softShadows: this.viewer.scene.shadowMap.softShadows,
			normalOffset: this.viewer.scene.shadowMap.normalOffset,
			fromLightSource: this.viewer.scene.shadowMap.fromLightSource
		})
		this.update();
	}

	updatebypositionend() {
		if (this.viewPositionEnd == null || this.viewPosition == null) return;
	...
	clear() {
		...
		if(this.originshadowmap!=null){
			this.viewer.scene.shadowMap=this.originshadowmap;
		}
	}
	...
	createPostStage() {
		if (this.postStage != null)
			return;
		// this.viewer.scene.postProcessStages.remove(this.postStage);
		this.postStage = this.viewer.scene.postProcessStages.add(new Cesium.PostProcessStage({
			fragmentShader: fs,
			uniforms: {
	...
	drawFrustumOutline() {
		...
		let position = this.lightCamera.positionWC;
		let direction = this.lightCamera.directionWC;
		let up = this.lightCamera.upWC;
			...
			new Cesium.Primitive({
				geometryInstances: [new Cesium.GeometryInstance({
					geometry: new Cesium.FrustumOutlineGeometry({
						frustum: this.lightCamera.frustum,
						origin: this.viewPosition,
						orientation: Cesium.Quaternion.fromRotationMatrix(rotation, scratchOrientation)
					}),
					id: Math.random().toString(36).substr(2),
					attributes: {
						color: Cesium.ColorGeometryInstanceAttribute.fromColor(
	...
	drawSketch() {
        ...
        // if (this.wedge != null) {
        //     this.viewer.entities.remove(this.wedge);
        // }
		...
		this.sketch = this.viewer.entities.add({
			...
			ellipsoid: {
				...
				// minimumClock: Cesium.Math.toRadians(-this.horizontalViewAngle / 2+180),
				// maximumClock: Cesium.Math.toRadians(this.horizontalViewAngle / 2+180),
...
export default {ViewShed: Viewshed};
