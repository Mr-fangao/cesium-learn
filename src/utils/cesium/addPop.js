...
export function addMapPop(viewer, option) {
	...
	let animateEvent = function (scene, time) {
		let position = Cesium.Cartesian3.fromDegrees(parseFloat(option.position[0]), parseFloat(option.position[1]),option[2]?parseFloat(option[2]):0);
	...
	//且这里还有东西没有删除
	const closepop=htmlOverlay.querySelector(".close-ignore");
	if(closepop){
		closepop.onclick = function () {
			viewer.scene.postUpdate.removeEventListener(animateEvent);
			viewer.cesiumWidget.container.removeChild(htmlOverlay);
			// Bus.VM.$emit(Bus.SignalType.CLOSEPOPUP,{id:option.id,index:option.index});
		};
	}
...
export function removeMapPop(viewer, eventList,selector) {
