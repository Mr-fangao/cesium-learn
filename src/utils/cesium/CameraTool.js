...
export function animateZoom(viewer, options) {
	if (!Cesium.defined(options.parentExtent) && !Cesium.defined(options.targetExtent)) {
		return;
	}
	...
	let duration = Cesium.defaultValue(options.duration, 0.5);
	let heading = Cesium.defaultValue(options.heading, 0);
	let pitch = Cesium.defaultValue(options.pitch, -30);
	let radius = Cesium.defaultValue(options.radius, 3);
	let parentSphere = Cesium.BoundingSphere.fromRectangle3D(
	...
	let targetSphere = Cesium.BoundingSphere.fromRectangle3D(
		Cesium.Rectangle.fromDegrees(
	...
	viewer.camera.flyToBoundingSphere(parentSphere, {
		duration: duration / 2,
		offset: new Cesium.HeadingPitchRange(Cesium.Math.toRadians(heading - 60), Cesium.Math.toRadians(-60), parentSphere.radius * radius),
		complete: () => {
...
export function fitBounds(viewer, extent, padding) {
	...
	padding = Cesium.defaultValue(padding, [0, 0, 0, 0]);
	let rectangle = Cesium.Rectangle.fromDegrees(extent[0], extent[1], extent[2], extent[3]);
	viewer.camera.flyTo({
		destination: rectangle,
		duration: 0.5,
		complete: function () {
			let resolution = getResolution(viewer);
			let topOffset = Cesium.defaultValue(meterToDegree(padding[0] * resolution), 0);
			let rightOffset = Cesium.defaultValue(meterToDegree(padding[1] * resolution), 0);
			let bottomOffset = Cesium.defaultValue(meterToDegree(padding[2] * resolution), 0);
			let leftOffset = Cesium.defaultValue(meterToDegree(padding[3] * resolution), 0);
			let bound = [extent[0] - 2 * leftOffset, extent[1] - 2 * bottomOffset, extent[2] + 2 * rightOffset, extent[3] + 2 * topOffset];
			let rectangle2 = Cesium.Rectangle.fromDegrees(bound[0], bound[1], bound[2], bound[3]);
			viewer.camera.flyTo({
...
function getResolution(viewer) {
	...
	let left = scene.camera.getPickRay(new Cesium.Cartesian2((width / 2) | 0, (height - 1) / 2));
	let right = scene.camera.getPickRay(new Cesium.Cartesian2((1 + width / 2) | 0, (height - 1) / 2));
